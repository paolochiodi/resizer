...
describe("Contain", function() {
  ...
  it("with only height should generate the parameters correctly", function() {
    ...
    expect(resizer.getParameters()).to.deep.equal(['-', '-size', 'x100>', '-thumbnail', 'x100>', '-quality', '91', '+profile', '*', '-auto-orient', '-strip', '-']);
  });

  it("with only width should generate the parameters correctly", function() {
    var resizer = new Contain({ width: 200 });
    expect(resizer.getParameters()).to.deep.equal(['-', '-size', '200x>', '-thumbnail', '200x>', '-quality', '91', '+profile', '*', '-auto-orient', '-strip', '-']);
  });
